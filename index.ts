 ...
 */
export const encode = (lat: number, lon: number, precision: number): string => {
  // infer precision?
 ...
 */
export const decode = (geohash: string): { lat: number; lon: number } => {
  const calculated_bounds = bounds(geohash); // <-- the hard work
  // now just determine the centre of the cell...

  const latMin = calculated_bounds.sw.lat,
    lonMin = calculated_bounds.sw.lon;
  const latMax = calculated_bounds.ne.lat,
    lonMax = calculated_bounds.ne.lon;
  ...
  const sLat = lat.toFixed(
    Math.floor(2 - Math.log(latMax - latMin) / Math.LN10)
  );
  const sLon = lon.toFixed(
    Math.floor(2 - Math.log(lonMax - lonMin) / Math.LN10)
  );
 ...
 * @throws  Invalid geohash.
 */
export const neighbours = (geohash: string) => {
  return {
